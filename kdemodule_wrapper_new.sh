#!/bin/bash
# ModuLLe KDE Global Shortcut Wrapper
#
# This script integrates ModuLLe with KDE Plasma global shortcuts.
# It captures selected text, prompts for an AI task, and displays results in a chat GUI.
#
# Setup Instructions:
# 1. Run setup.sh to install dependencies
# 2. Make this script executable: chmod +x kdemodule_wrapper_new.sh
...
LAUNCHER="$SCRIPT_DIR/modulle-chat.sh"
...
if [ ! -d "$VENV_DIR" ]; then
    kdialog --error "ModuLLe is not set up. Please run:\n\ncd $SCRIPT_DIR\n./setup.sh"
    exit 1
fi
...
export MODULLE_INITIAL_TEXT="$SELECTED_TEXT"
export MODULLE_INITIAL_PROMPT="$USER_PROMPT"
